...
<body>
	...
	<br>
	<div class="container">
		<form action="<c:url value="./M_NoticeListAction.do"/>" method="post">
			<!-- 검색 데이터 값을 기록하는 부분 -->
			<input type="hidden" name="newSearch" value="yes" />

			<div>
				<div class="text-right">
					<span class="badge badge-dark"> 전체 ${total_record}개 </span>
				</div>
			</div>
				...
				<table class="table table-hover">
					...
					<c:forEach var="item" items="${noticelist}">
						<tr>
							...
							<!-- 제목 -->
							<td>
								<a href="./M_NoticeViewAction.do?num=${item.n_num}&pageNum=${pageNum}">
									${item.subject}</a>
							</td>
							<!-- 작성일 -->
			...
			<div align="center">
				...
				<c:forEach var="i" begin="1" end="${total_page}">
					<a href="<c:url value="./M_NoticeListAction.do?pageNum=${i}" /> ">
						<c:choose>
			...
			<div class="btn-group btn-group-toggle" data-toggle="buttons" align="right">
				<label class="btn"> <select name="items" class="txt btn btn-outline-primary">
						...
						type="submit" id="btnAdd" class="btn btn-outline-primary btn-sm" value="검색 "
					/>
				</label>
			</div>
